...
func (m *Minerador) Update() error {
	...
	_, err = db.Exec(
		"UPDATE minerador SET Especialidade = :1 WHERE ID = :2",
		m.Especialidade, m.ID,
	)
	return err
}
...
func ListMineradores() ([]Minerador, error) {
	db := config.GetDB()
	rows, err := db.Query("SELECT m.ID, m.Especialidade, h.Salario, h.Nome, h.Genero, h.IDColonia FROM minerador m LEFT JOIN humano h ON m.ID = h.ID ORDER BY ID ASC")
	if err != nil {
		return nil, err
	}
	defer rows.Close()
