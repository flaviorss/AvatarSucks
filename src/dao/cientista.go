...
func (c *Cientista) Update() error {
    db := config.GetDB()
    _, err := db.Exec(
        "UPDATE cientista SET Salario = :1, Nome = :2, Genero = :3, IDColonia = :4, AreaAtuacao = :5 WHERE ID = :6",
...
func ListCientistas() ([]Cientista, error) {
    db := config.GetDB()
    rows, err := db.Query("SELECT c.ID, c.AreaAtuacao, h.Salario, h.Nome, h.Genero, h.IDColonia FROM cientista c LEFT JOIN humano h ON c.ID = h.ID ")
    if err != nil {
        return nil, err
    }
    defer rows.Close()
